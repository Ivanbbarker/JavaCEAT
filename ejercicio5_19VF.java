...
public class ejercicioApliaccion5_19 {
    public static void main(String[] args) {
        System.out.println(" ---- GESTION DE CARRERAS ----");
        int t[] = llegadaCorredores();
        mostrarClasificacion(t);
        System.out.println("\nNorma 1ª, los participantes menores de edad avanzan una posicion.\n");

        t = menoresEdad(t);
        mostrarClasificacion(t);
        System.out.println("\nNorma 2ª los participantes que den positivo en el test de dopaje, quedaran EXPLUSADOS.\n");
        t = dopados(t);
        mostrarClasificacion(t);
        System.out.println("\nNorma 3ª, los participantes que no hayan pagado su inscripcion, serán ubicados al final de la clasificación.\n");
        t = sinInscripcion(t);
    ...
    public static int[] dopados(int t[]) {
        ...
        do {
            ...
            if (indiceBusqueda == t.length) {
                ...
                System.out.println("Dorsal numero: [" + t[indiceBusqueda] + "] eliminado de la carrera.");
    ...
    public static int[] sinInscripcion(int t[]) {
        ...
        int dorsal;
        do {
            System.out.print("Introduce el dorsal de los participantes sin inscripcion: ");
            dorsal = sc.nextInt();
            ...
            if (indiceBusqueda == t.length) {
                ...
                t[t.length - 1] = aux;
                System.out.println("Dorsal numero: [" + aux + "], introducido al final.");
            }
        } while (dorsal != -1);
        return t;
    }
    ...
    public static int[] menoresEdad(int t[]) {
        ...
        int dorsal, aux = 0;
        do {
            System.out.print("Introduce el dorsal del participante menor de edad: ");
            dorsal = sc.nextInt();
            int indiceBusqueda = 0;
            while (indiceBusqueda < t.length && t[indiceBusqueda] != dorsal) {
                indiceBusqueda++;
            ...
            if (indiceBusqueda == t.length) {
                ...
                for (int i = indiceBusqueda; i < indiceBusqueda + 1; i++) {
                    if (i == 0) {
                        System.out.println("Ese participante no puede avanzar mas porque ya esta en el primer lugar.");
                    } else {
                        aux = t[i];
                        t[i] = t[i - 1];
                        t[i - 1] = aux;
